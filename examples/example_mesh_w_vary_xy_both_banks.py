...
tds.add_sp(sp, x=0)
tds.add_sp(sp2, x=17)
tds.x_surf = np.array([0, 10, 12, 15, 17, 35, tds.width])
tds.y_surf = np.array([h_face, h_face, 0, 0, h_face, h_face+0.6, h_face + 0.6])
# tds.x_surf = np.array([0, 20, 21, tds.width])
# tds.y_surf = np.array([h_face-0.9, h_face, 0, 0.])

x_scale_pos = np.array([0, 5, 30])
x_scale_vals = np.array([2., 1.5, 2])
...
fc = mesh2d_vary_y.FiniteElementVary2DMeshConstructor(tds, 0.5, x_scale_pos=x_scale_pos,
                                                       x_scale_vals=x_scale_vals, auto_run=False, smooth_ratio=1.5)
